...
def ComputeEuclid1(array1,array2,fg_sqrt=True,fg_norm=False):
...
def ComputeEuclid2(array1, array2, fg_sqrt=True, fg_norm=False):
    # array1:[m1,n],array2:[m2,n]

    # Move data to GPU
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    if isinstance(array1, np.ndarray):
        array1 = torch.tensor(array1, dtype=torch.float64).to(device)
        array2 = torch.tensor(array2, dtype=torch.float64).to(device)
    else:
        array1 = array1.to(torch.float64)
        array2 = array2.to(torch.float64)
    ...
    # Norm
    if fg_norm:
        array1 = F.normalize(array1, p=2, dim=1)  # Use F.normalize for L2 normalization
        array2 = F.normalize(array2, p=2, dim=1)

    # Shape [m1, 1]
    # square1 = torch.sum(array1 ** 2, dim=1, keepdim=True)  # Use torch.sum for sum along axis 1
    # # Shape [1, m2]
    # square2 = torch.sum(array2 ** 2, dim=1, keepdim=True).T  # Use torch.sum for sum along axis 1, transpose
    # # print(square1, square2)
    # # print('array1,array2 shape:',array1.shape,array2.shape)
    # squared_dist = -2 * torch.mm(array1, array2.T) + square1 + square2  # Use torch.mm for matrix multiplication
    # squared_dist[squared_dist < 0] = 0
    ...
    #     dist = squared_dist
    #     # print('[test] not using sqrt for distance')
    dist = torch.cdist(array1, array2, p = 2)

    sim = 1 - dist ** 2 / 2.0
    return 1 - sim.cpu().numpy()  # Return result to CPU as NumPy array
...
def ReRank(probFea,galFea,k1=20,k2=6,lambda_value=0.3):
    ...
    original_dist = ComputeEuclid2(feat, feat, fg_sqrt=False, fg_norm=True)
